...
new_base_dir = 'fer2013_original_renamed'  # New base directory for renamed files
dataset_dirs = {'train': 'train', 'val': 'val', 'test': 'test'}  # Mapping original to new names

# Create new directory structure
for original_split, new_split in dataset_dirs.items():
    split_path = os.path.join(new_base_dir, new_split)
    os.makedirs(split_path, exist_ok=True)
    # Only create subdirectories if the original directory exists (handles val correctly)
