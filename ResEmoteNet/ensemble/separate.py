...
# Create the validation directory structure if it doesn't exist
if not os.path.exists(validation_dir):
    os.makedirs(validation_dir)
    for class_name in classes:
        os.makedirs(os.path.join(validation_dir, class_name))

# Function to move PublicTest images to validation directory
    ...
    for class_name in classes:
        ...
        if os.path.exists(test_class_dir):
            ...
            for filename in os.listdir(test_class_dir):
                if filename.startswith("PublicTest_"):
                    # Move the file to the corresponding validation subdirectory
